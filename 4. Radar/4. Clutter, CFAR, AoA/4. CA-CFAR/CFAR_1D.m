...
% Targets location. Assigning bin 100, 200, 300 and 700 as 
% targets with the amplitudes of 8, 9, 4, 11.
...
for i = 1:(Ns-(G+T))     
    ...
    threshold = (noise_level / T) * offset;  % check both */+ for offset
    threshold_cfar = [threshold_cfar, {threshold}];

    % 6. Measuring the signal within the cell under test (CUT)
    signal = s(i+T+G);
