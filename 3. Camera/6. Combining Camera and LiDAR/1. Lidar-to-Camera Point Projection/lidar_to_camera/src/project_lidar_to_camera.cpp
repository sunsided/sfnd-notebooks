...
void projectLidarToCamera2()
{
    ...
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {
        ...
        // The spinning LiDAR sensor captures points behind the camera.
        // When projecting 3D points into the image plane, they will generate a negative X coordinate,
        // which will color them red.
        //
        // Specifically, we will filter out point that are ...
        ...
        const auto maxX = 25.0F, maxY = 6.0F, minZ = -1.4F;

        const auto behindCamera = it->x < 0.0;
        const auto tooFarAway = it->x > maxX;
        const auto tooFarAwayLaterally = abs(it->y) > maxY;
        const auto tooCloseToTheRoad = it->z < minZ;
        const auto reflectivityTooLow = it->r < 0.01;

        if(tooFarAway || behindCamera || tooFarAwayLaterally || tooCloseToTheRoad || reflectivityTooLow )
        {
    ...
    cv::imwrite("projected.jpg", visImg);
}
